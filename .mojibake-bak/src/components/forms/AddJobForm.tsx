 ...
 * Bambeh Marketplace â€” Add Job Listing Form
...
const JOB_TYPES = ["CDI", "CDD", "Temps partiel", "Stage", "Freelance", "BÃ©nÃ©volat"] as const;
const JOB_CATEGORIES = ["Informatique & Tech", "Commerce & Vente", "Ã‰ducation", "SantÃ©", "Transport", "Agriculture", "BTP & Travaux", "Administration", "HÃ´tellerie & Restauration", "Autres"] as const;
...
const AddJobForm: React.FC<AddJobFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) { setError("Vous devez Ãªtre connectÃ©."); return; }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Trouvez le talent idÃ©al pour votre entreprise</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="Ex: DÃ©veloppeur React Senior" required className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none" />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="DÃ©crivez le poste, les responsabilitÃ©s, l'environnement de travail..." rows={5} required className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none resize-none" />
        </div>

        {/* Requirements */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">PrÃ©requis & Qualifications</label>
          <textarea value={form.requirements} onChange={(e) => set("requirements", e.target.value)} placeholder="Niveau d'Ã©tudes, expÃ©riences requises, compÃ©tences..." rows={3} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none resize-none" />
        </div>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-700">TÃ©lÃ©travail possible</span>
          </label>
        </div>
