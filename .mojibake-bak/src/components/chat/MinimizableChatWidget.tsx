/**
 * MINIMIZABLE CHAT WIDGET â€” Floating chat support widget.
 */
...
const MinimizableChatWidget = () => {
  ...
  const sendMessage = () => {
    ...
    if (trimmedInput) {
      setMessages(prev => [...prev, { id: Date.now().toString(), text: trimmedInput, isUser: true }]);
      setInputValue('');
    }
  };
    ...
    return (
      <div className="fixed bottom-6 right-6 z-[9999]">
        ...
        >
          ...
          <span className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-xs font-bold rounded-full flex items-center justify-center shadow-lg animate-pulse">1</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] w-96 max-w-[95vw] animate-in slide-in-from-bottom-2 duration-200">
      <div className={`bg-white rounded-3xl shadow-2xl border overflow-hidden transition-all duration-300 ${isMinimized ? 'h-16' : 'h-[500px] max-h-[80vh]'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-4 rounded-2xl text-sm ${msg.isUser ? 'bg-emerald-500 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
                    {msg.text}
                  </div>
                </div>
              ))}
