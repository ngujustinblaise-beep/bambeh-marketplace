 ...
 *
 * ORDER-FIRST VERSION (audit P0 #1 fix):
 *  - The client NO LONGER creates orders. The old onSuccess handler that
 *    inserted a 'paid' order into Supabase from the browser is GONE.
 *  - "Pay with Mobile Money" now calls initCartPayment: the SERVER verifies
 *    prices, reserves stock, creates the order as 'pending', initiates
 *    CamPay, and the signature-verified webhook flips it to 'paid' and
 *    opens the escrow ledger row.
 *  - Checkout requires a signed-in buyer (redirects to /login if not).
 *  - "Pay via Escrow" and "More Payment Options" keep navigating to
 *    /payment/checkout exactly as before.
 *  - Destroyed emoji characters ("??") restored throughout.
 */
...
export default function Cart() {
  ...
  // -- useCamPay hook -----------------------------------------------------------
  // ORDER-FIRST: the server creates the order BEFORE money moves and the
  // webhook marks it paid. The client no longer writes orders at all.
  const { status, errorMsg, reference, countdown, initCartPayment, reset } = useCamPay({
    onSuccess: async () => {
      clearCart();
  ...
  // -- Initiate mobile money payment (order-first, authenticated) ----------------
  const handlePay = useCallback(async (phone: string) => {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.access_token) {
      // Checkout requires a signed-in buyer - send them to login and back.
      setShowMobileMoney(false);
    ...
    await initCartPayment({
      items: items.map(i => ({
        ...
        listingType: i.listingType ?? null,
  ...
  }, [initCartPayment, items, cartDescription, navigate]);
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-8">
        ...
        <p className="text-gray-500 mb-1 text-sm text-center">
          {escrowDone
            ...
            : 'Payment confirmed! Your order is protected by Bambeh escrow.'}
        </p>
        {reference && (
