import {
  ...
  settlementLedger,
  assertBalanced,
} from "../src/services/payment/bambehPricing.ts";
...
check("buyer pays", q.buyerTotalXaf, 1042);
check("seller amount", q.sellerAmountXaf, 1000);
check("commission (1% + 4 flat)", q.commissionXaf, 14);
check("VAT (exact micro)", q.exactMicro.vat, 2_695_000);
check("gov tax both sides", q.govTaxXaf, 4);
...
check("buyer pays", ql.buyerTotalXaf, 1042);
check("Bambeh keeps after paying the seller", kept, 3);
console.log("  ^^ 3 XAF left out of a 14 XAF commission once the payout fee is absorbed.");
...
console.log("\n=== 7. Webhook guard ===");
check("exact amount", verifyPaidAmount(qb, 1053).ok, true);
check("one franc short", verifyPaidAmount(qb, 1052).ok, false);
check("short, with tolerance", verifyPaidAmount(qb, 1052, 1).ok, true);
...
check("at 1.5% collection", quoteCheckout(1000, negotiated).buyerTotalXaf, 1037);
const richer = { ...BAMBEH_PRICING, commissionBp: 300, payoutFeePaidBy: "seller" as const };
check("at 3% commission", quoteCheckout(1000, richer).buyerTotalXaf, 1066);
