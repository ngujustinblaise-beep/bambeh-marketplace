...
export const ChatProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // FIX: all 4 handlers had `);` instead of `});`
    ...
    return () => {
      newSocket.close();
    }; // FIX: missing }; for useEffect return/cleanup
  }, []);
  ...
  }; // FIX: was missing closing }; for handleIncomingMessage
  ...
  const sendMessage = (messageData: Omit<ChatMessage, "id" | "timestamp" | "read">) => {
    ...
    setConversations((prev) => {
      const existingIndex = prev.findIndex((c) => c.id === messageData.conversationId);
      if (existingIndex >= 0) {
        const updated = [...prev];
        updated[existingIndex] = {
          ...updated[existingIndex],
    ...
    } // FIX: missing }; closing the if block inside sendMessage
  };
  ...
  }; // FIX: was missing closing }; for markAsRead
  ...
  }; // FIX: was missing closing }; for deleteConversation
  ...
  }; // FIX: was missing closing }; for searchConversations

  const getConversationMessages = (conversationId: string): ChatMessage[] => {
    return messages
      .filter((m) => m.conversationId === conversationId)
      .sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
  }; // FIX: was missing closing }; for getConversationMessages
