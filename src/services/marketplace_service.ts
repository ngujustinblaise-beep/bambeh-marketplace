/**
 * src/services/marketplace_service.ts — Bambeh Marketplace
 *
 * REWRITE — June 2026
 *
 * FIXES:
 *  ✅ Uses `listings` table (canonical)
 *  ✅ increment_view_count RPC passes table_name + record_id
 *  ✅ All CRUD uses seller_id (not user_id / vendor_id)
 *  ✅ No reference to removed marketplace_items table
 *  ✅ Listings expire 30 days after creation (stored in expires_at)
 *  ✅ getExpiringListings() helper for push notification service
 *  ✅ Platform fee: 1% deducted and tracked in payment_transactions
 *
...
/** Calculate 1% Bambeh platform fee */
export function platformFee(priceXAF: number): number {
  return Math.max(1, Math.round(priceXAF * 0.01));
}
...
export async function getMarketplaceListings(
  ...
  try {
    ...
    if (filters.searchQuery) query = query.or(`title.ilike.%${filters.searchQuery}%,description.ilike.%${filters.searchQuery}%,location.ilike.%${filters.searchQuery}%`);
...
export async function getSellerListings(sellerId: string): Promise<PaginatedResponse<MarketplaceListingRow>> {
  try {
    ...
    return { data: (data ?? []) as MarketplaceListingRow[], total: count ?? 0, page: 1, pageSize: 100, hasNextPage: false, error: null };
  } catch (err) {
...
export async function renewListing(id: string, sellerId: string): Promise<ServiceResponse<null>> {
  try {
    const { error } = await supabase
      .from("listings")
      .update({ expires_at: expiryDate(), status: "active", updated_at: new Date().toISOString() })
      .eq("id", id)
      .eq("seller_id", sellerId);
