...
class ExchangeService {
  async getAllExchangeRequests(filters?: { type?: string }): Promise<ExchangeRequest[]> {
    try {
      ...
      if (filters?.type) {
        return exchanges.filter((e: ExchangeRequest) => e.type === filters.type);
      }
      return exchanges;
    }
  }
  ...
  async createExchangeRequest(
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/createExchangeRequest`, requestData, {
        headers,
        timeout: 5000,
      });
  ...
  async respondToExchange(exchangeId: string, message: string): Promise<void> {
    try {
      ...
      await axios.post(`${API_BASE_URL}/respondToExchange`, { exchangeId, message }, { headers, timeout: 5000 });
    } catch (error: any) {
      ...
      allResponses.push({ exchangeId, message, respondedAt: new Date().toISOString() });
      localStorage.setItem("bambeh_local_exchange_responses", JSON.stringify(allResponses));
    }
  }
}
...
class ReferralService {
  async getReferralCode(): Promise<string> {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/getReferralCode`, { headers, timeout: 5000 });
  ...
  async getMyReferrals(): Promise<Referral[]> {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/getMyReferrals`, { headers, timeout: 5000 });
  ...
  async getReferralStats(): Promise<ReferralStats> {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/getReferralStats`, { headers, timeout: 5000 });
  ...
  async sendReferralInvite(email: string): Promise<void> {
    try {
      ...
      await axios.post(`${API_BASE_URL}/sendReferralInvite`, { email }, { headers, timeout: 5000 });
    } catch (error: any) {
  ...
  async claimReferralReward(referralId: string): Promise<void> {
    try {
      ...
      await axios.post(`${API_BASE_URL}/claimReferralReward`, { referralId }, { headers, timeout: 5000 });
    } catch (error: any) {
