// BAMBEH_DEPLOY_TOKEN__USESUBSCRIPTION_FIX91_CLEAN
...
async function verifyWithSupabase(userId: string): Promise<CachedSub | null> {
  ...
  if (error) {
    // Offline/transient (Cameroon networks): keep the last known answer for
    // this user rather than kicking them out mid-session.
