...
class NLPService {
  ...
  private getCommandPatterns(language: Language): {
    ...
    if (language === "fr") {
      return {
        ...
        order: [
          /commande|commander|ach�te|acheter|prends|prendre/i,
          /je veux commander|je voudrais commander/i,
          /ajouter au panier|mettre dans le panier/i,
        ],
        track: [
          /o� est|suivre|traquer|localiser/i,
        ...
        navigate: [
          /va �|aller �|ouvre|ouvrir|affiche|afficher/i,
          /montre moi la page|emm�ne moi/i,
        ],
        cancel: [
          /annule|annuler|arr�te|arr�ter|stop/i,
          /non merci|laisse tomber/i,
        ],
        confirm: [/oui|d'accord|ok|confirme|confirmer/i, /vas-y|go|allez-y/i],
        repeat: [
          /r�p�te|r�p�ter|redis|redire|encore/i,
    ...
    const categoryKeywords =
      language === "fr"
        ? {
            ...
            electronics: ["�lectronique", "appareil", "gadget"],
            fashion: ["mode", "v�tement", "habit"],
            beauty: ["beaut�", "cosm�tique", "maquillage"],
            home: ["maison", "jardin", "d�coration"],
  ...
  private extractQuantities(text: string, language: Language): CommandEntity[] {
    ...
    // Match numbers
    const numberPattern = /(\d+)\s*(kilo|kg|litre|l|pi�ce|piece|unit|unit�)?/gi;
    ...
    if (language === "fr") {
      switch (command.command) {
        case "search":
          return [
            "Cherche des tomates",
            "Trouve des t�l�phones",
            "Montre moi des v�tements",
          ];
        case "order":
          return [
            "Commander du pain",
