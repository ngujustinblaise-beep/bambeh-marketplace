// BAMBEH_DEPLOY_TOKEN__BOOKVISITMODAL_FIX83_CLEAN
...
export default function BookVisitModal({ isOpen, onClose, listing, onSuccess }: Props) {
  ...
  const ownerId = listing.user_id || listing.owner_id || listing.creator_id || '';
  const adTitle = listing.title || listing.listing_title || listing.name || 'Listing';
      ...
      >
        <div className="flex items-start justify-between bg-gradient-to-r from-teal-600 to-cyan-600 px-5 py-4 text-white">
          <div className="pr-4">
            ...
            <h2 className="mt-3 text-xl font-bold leading-tight">{t('book.visit.title')}</h2>
            <p className="mt-1 text-sm text-teal-50">{adTitle}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5 sm:px-6">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={!canSubmit} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-teal-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60"><Loader2 className={`h-4 w-4 ${loading ? 'animate-spin' : 'hidden'}`} />{loading ? t('common.sending') : t('book.visit.submit')}</button>
          </div>
        </form>
      </div>
    </div>
...
// BAMBEH_END_TOKEN__BOOKVISITMODAL_FIX83__COMPLETE
