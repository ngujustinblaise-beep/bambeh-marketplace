// BAMBEH_DEPLOY_TOKEN__BOOKTESTDRIVEMODAL_FIX83_CLEAN
...
export default function BookTestDriveModal({ isOpen, onClose, listing, onSuccess }: Props) {
  ...
  const ownerId = listing.seller_id || listing.user_id || listing.owner_id || listing.creator_id || '';
  const adTitle = listing.title || listing.listing_title || listing.name || 'Vehicle';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sendBookingMessage({
        ...
        listingImage: listing.listingImage || listing.listing_image,
      });
      await onSuccess?.();
      onClose();
      setDate('');
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      >
        <div className="flex items-start justify-between bg-gradient-to-r from-amber-500 to-orange-600 px-5 py-4 text-white">
          <div className="pr-4">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold backdrop-blur">
              <CarFront className="h-3.5 w-3.5" />
              {t('book.testDrive.badge')}
            </div>
            <h2 className="mt-3 text-xl font-bold leading-tight">{t('book.testDrive.title')}</h2>
            <p className="mt-1 text-sm text-amber-50">{adTitle}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5 sm:px-6">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={!canSubmit} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-orange-700 disabled:cursor-not-allowed disabled:opacity-60"><Loader2 className={`h-4 w-4 ${loading ? 'animate-spin' : 'hidden'}`} />{loading ? t('common.sending') : t('book.testDrive.submit')}</button>
          </div>
        </form>
      </div>
    </div>
...
// BAMBEH_END_TOKEN__BOOKTESTDRIVEMODAL_FIX83__COMPLETE
