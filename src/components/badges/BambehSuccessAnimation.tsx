...
function BambehFigureSVG({ phase }: { phase: number }) {
  return (
    <svg
      ...
      height="192"
      style={{
        filter: 'drop-shadow(0 8px 24px rgba(184,150,12,0.5))',
        transition: 'transform 0.4s ease',
        transform: phase === 1 ? 'scaleX(1.05)' : 'scaleX(1)',
      }}
    >
      <defs>
        <radialGradient id="sa-skin" cx="50%" cy="40%" r="60%">
          <stop offset="0%" stopColor="#D4A017" />
          <stop offset="100%" stopColor="#8B6F0A" />
        </radialGradient>
        <radialGradient id="sa-load" cx="50%" cy="50%" r="55%">
          <stop offset="0%" stopColor="rgba(255,248,231,0.25)" />
          <stop offset="100%" stopColor="rgba(255,248,231,0.08)" />
        </radialGradient>
      </defs>
      ...
      {phase === 0 && (
        <g>
          ...
          <rect x="52" y="18" width="14" height="10" rx="2" fill="rgba(255,255,255,0.12)" stroke="#D4A017" strokeWidth="1.2" />
          <circle cx="40" cy="13" r="6" fill="rgba(255,255,255,0.12)" stroke="#B8960C" strokeWidth="1.2" />
          <circle cx="50" cy="42" r="10" fill="url(#sa-skin)" />
          <circle cx="46" cy="40" r="1.5" fill="#3D2800" />
          <circle cx="54" cy="40" r="1.5" fill="#3D2800" />
          ...
          <ellipse cx="38" cy="122" rx="10" ry="5" fill="#8B6F0A" />
          <ellipse cx="62" cy="122" rx="10" ry="5" fill="#8B6F0A" />
        </g>
      )}
      {phase === 1 && (
        <g>
          <g style={{ transform: 'translateY(30px) rotate(8deg)', transformOrigin: '50px 30px' }}>
            ...
            <rect x="26" y="36" width="18" height="12" rx="3" fill="rgba(255,255,255,0.12)" stroke="#B8960C" strokeWidth="1.2" opacity="0.7" />
          </g>
          ...
          <ellipse cx="30" cy="126" rx="10" ry="5" fill="#8B6F0A" />
          <ellipse cx="70" cy="126" rx="10" ry="5" fill="#8B6F0A" />
          <path d="M64 46 Q65 42 66 44 Q67 46 65.5 47 Z" fill="#B8D4FF" opacity="0.7" />
        </g>
      )}
      {phase === 2 && (
        <g>
          ...
          <ellipse cx="40" cy="118" rx="10" ry="5" fill="#8B6F0A" />
          <ellipse cx="60" cy="118" rx="10" ry="5" fill="#8B6F0A" />
          <path d="M30 26 Q29 22 30 20 Q32 22 31 25 Z" fill="#B8D4FF" opacity="0.6" />
        </g>
      )}
      {phase === 3 && (
        <g>
          ...
          <path d="M44 41 Q50 46 56 41 Q54 44 50 44.5 Q46 44 44 41 Z" fill="white" opacity="0.8" />
          ...
          <ellipse cx="41" cy="116" rx="10" ry="5" fill="#8B6F0A" />
    ...
    <div style={containerStyle}>
      <style>{`
        ...
        .sa-ring {
          ...
          animation: sa-ring-expand 1.2s 0.4s ease-out both;
        }
        ...
        .sa-dismiss-btn {
          ...
          animation: sa-slide-up 0.6s 0.8s ease both;
        }
        ...
        return (
          <div
            ...
            style={{
              ...
              top: `${pos.y}%`,
              width: pt.size,
              height: pt.shape === 'circle' ? pt.size : pt.size * 0.5,
              borderRadius: pt.shape === 'circle' ? '50%' : pt.shape === 'star' ? '0' : '1px',
              background: pt.color,
              transform: `rotate(${pos.rotation}deg)`,
              opacity: Math.max(0, Math.min(1, 1 - (pos.y - 70) / 20)),
              pointerEvents: 'none',
              fontSize: pt.shape === 'star' ? pt.size : undefined,
      ...
      <div style={{
        position: 'relative',
        textAlign: 'center',
        padding: isOverlay ? '48px 40px 36px' : '32px 24px',
        borderRadius: 24,
        background: isOverlay
          ? 'linear-gradient(160deg, #1a1200 0%, #0f0a00 100%)'
          : 'transparent',
        border: isOverlay ? '1px solid rgba(184,150,12,0.3)' : 'none',
        maxWidth: 360,
        width: '100%',
      }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', pointerEvents: 'none' }}>
          <div className="sa-ring" />
          <div className="sa-ring sa-ring-2" />
        </div>
...
export function useBambehSuccess() {
  ...
  const triggerSuccess = (opts: { message?: string; subMessage?: string } = {}) => {
    setConfig({
      visible: true,
      message: opts.message ?? 'Load delivered! You did it, partner.',
      subMessage: opts.subMessage,
    });
  };

  const SuccessAnimation = () => (
    <BambehSuccessAnimation
