...
import {
  Upload, X, MapPin, DollarSign, Package, Tag, Loader2,
  ShoppingCart, Check, AlertCircle, Image as ImageIcon,
  Info, Camera, Star, TrendingUp
} from 'lucide-react';
...
const PostMarketplaceItem = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name.includes('.')) {
      ...
      setFormData(prev => ({
        ...prev,
        [parent]: {
          ...(prev[parent as keyof MarketplaceItem] as object),
          [child]: value
        }
      }));
    } else {
  ...
  const handleFiles = (files: FileList) => {
    ...
    if (images.length + fileArray.length > 10) return alert('Max 10 images');
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-6 animate-fadeIn">
                <header className="border-b pb-4">
                  <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    <Package className="text-blue-600" /> Basic Information
                  </h2>
                </header>
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full p-4 border-2 border-gray-100 rounded-2xl focus:border-blue-500 outline-none transition-all"
                    placeholder="Provide details about condition, age, and features..."
                  />
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                <header className="border-b pb-4">
                  <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    <DollarSign className="text-green-600" /> Pricing & Visuals
                  </h2>
                </header>
                ...
                <div className="grid grid-cols-5 gap-4">
                  {imagePreviews.map((p, i) => (
                    <div key={p.id} className="relative group aspect-square rounded-xl overflow-hidden shadow-sm">
                      ...
                      <button type="button" onClick={() => setImagePreviews(prev => prev.filter((_, idx) => idx !== i))} className="absolute top-1 right-1 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <header className="border-b pb-4">
                  <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    <MapPin className="text-red-600" /> Logistics & Contact
                  </h2>
                </header>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                ...
                <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      name="acceptsZermCoins"
                      checked={formData.acceptsZermCoins ?? true}
                      onChange={handleInputChange}
            ...
            {currentStep < 4 && (
              <div className="flex gap-4 mt-12">
                {currentStep > 1 && (
                  <button type="button" onClick={() => setCurrentStep(prev => prev - 1)} className="px-8 py-4 border-2 border-gray-100 rounded-2xl font-bold hover:bg-gray-50">Back</button>
                )}
                <button
