...
const Logo: React.FC<LogoProps> = ({
  ...
  const sizes = {
    sm:  { container: 'h-8',  icon: 'w-8 h-8',   text: 'text-xl',  tagline: 'text-xs',  logoText: 'text-lg'  },
    md:  { container: 'h-12', icon: 'w-12 h-12', text: 'text-2xl', tagline: 'text-sm',  logoText: 'text-xl'  },
    lg:  { container: 'h-16', icon: 'w-16 h-16', text: 'text-3xl', tagline: 'text-base',logoText: 'text-2xl' },
    xl:  { container: 'h-20', icon: 'w-20 h-20', text: 'text-4xl', tagline: 'text-lg',  logoText: 'text-3xl' },
  };
  const s = sizes[size];

  const GlossyTurquoiseLogo = () => (
    <div className={`${s.icon} relative rounded-2xl overflow-hidden shadow-2xl`}
      style={{ background: 'linear-gradient(135deg,#06b6d4 0%,#0891b2 50%,#0e7490 100%)', boxShadow: '0 10px 30px rgba(6,182,212,0.4),inset 0 1px 0 rgba(255,255,255,0.3)' }}>
