...
export default function LiveChatSupport({ buttonText = 'Start Live Chat', className = '' }: LiveChatSupportProps) {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const welcomeMessages: Message[] = [
        { id: 'welcome_1', text: `Hello${currentUser?.name ? ` ${currentUser.name}` : ''}! ðŸ’š Welcome to Bambeh Support!`, sender: 'bot', timestamp: new Date() },
  ...
  const getAutoResponse = (userMessage: string): string[] => {
    ...
    if (lc.match(/^(hi|hello|hey|good morning|good afternoon|good evening|bonjour|bonsoir)/)) {
      return ["Hello! ðŸ‘‹ Thank you for reaching out to Bambeh Support!", "How can I help you today? You can ask about buying, selling, payments, or any other questions!"];
    }
    if (lc.includes('subscription') || lc.includes('premium') || lc.includes('plan')) {
      return ["Great question about subscriptions! ðŸŒŸ", "Bambeh offers Free, Basic (1,500 XAF/month), Premium (2,500 XAF/month), and Business (5,000 XAF/month) plans.", "Each plan comes with different features. Would you like me to connect you with a support agent for more details?"];
    }
    if (lc.includes('payment') || lc.includes('pay') || lc.includes('momo') || lc.includes('orange money')) {
      return ["For payments, Bambeh accepts: ðŸ’³", "â€¢ MTN Mobile Money\nâ€¢ Orange Money\nâ€¢ Credit/Debit Cards", "We charge only 1% transaction fee - the lowest in ! ðŸ’š"];
    }
    if (lc.includes('sell') || lc.includes('list') || lc.includes('post')) {
      return ["Selling on Bambeh is easy! ðŸ“¦", "1. Create an account\n2. Click 'Post Ad'\n3. Add photos and description\n4. Set your price\n5. Publish!", "Free users can post 1 listing per month. Upgrade for unlimited listings!"];
    }
    if (lc.includes('buy') || lc.includes('purchase') || lc.includes('order')) {
      return ["Buying on Bambeh is safe and simple! ðŸ›’", "Browse products, contact sellers, and complete secure transactions through our escrow system.", "Need help finding something specific?"];
    }
    if (lc.includes('help') || lc.includes('support') || lc.includes('agent') || lc.includes('human') || lc.includes('person')) {
      ...
      return ["I understand you'd like to speak with a human agent. ðŸ‘¨Ã¢â‚¬ÂðŸ’¼", "I've notified our support team and someone will be with you shortly!", "Please stay on this chat. Average response time is 5-10 minutes during business hours (8 AM - 8 PM WAT). ðŸ•"];
    }
    if (lc.includes('zerm') || lc.includes('coin')) {
      return ["Zerm Coins are Bambeh's reward currency! ðŸª™", "You can earn them through:\nâ€¢ Daily logins (+50)\nâ€¢ Completing your profile (+200)\nâ€¢ Making purchases (+varies)\nâ€¢ Referrals (+500)", "Use Zerm Coins to boost listings, get discounts, and unlock premium features!"];
    }
    if (lc.includes('thank') || lc.includes('merci')) {
      return ["You're welcome! ðŸ’š It's my pleasure to help!", "Is there anything else I can assist you with?"];
    }
    return ["Thank you for your message! ðŸ“©", "I'm still learning, but I want to make sure you get the best help possible.", "Would you like me to connect you with a support agent who can assist you better?"];
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-hidden flex flex-col">
            ...
            <div className="border-t border-gray-200 p-4 bg-white flex-shrink-0">
              ...
              <p className="text-xs text-gray-400 text-center mt-2">Powered by Bambeh Support ðŸ’š</p>
            </div>
          </div>
        </div>
      )}
