 ...
 * Bambeh Marketplace â€” Post Job Listing Form
...
const JOB_CATEGORIES = [
  "Technologie & IT", "Commerce & Vente", "Ã‰ducation & Formation",
  "SantÃ© & MÃ©dical", "Construction & BTP", "Transport & Logistique",
  "Agriculture", "Finance & ComptabilitÃ©", "Marketing & Communication",
  "Juridique", "HÃ´tellerie & Restauration", "Autre",
];
...
const PostJobListing: React.FC<PostJobListingProps> = ({
  ...
  const validate = (): string | null => {
    ...
    if (form.title.trim().length < 5) return "Le titre doit contenir au moins 5 caractÃ¨res";
    ...
    if (form.description.trim().length < 30) return "Description trop courte (min 30 caractÃ¨res)";
    if (!form.city) return "Veuillez sÃ©lectionner une ville";
    return null;
  };
  ...
  if (success) {
    return (
      <div className={`text-center py-10 ${className}`}>
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-1">Offre publiÃ©e!</h2>
  ...
  return (
    <div className={`space-y-5 ${className}`}>
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="flex items-center border border-gray-300 rounded-xl overflow-hidden focus-within:border-teal-500">
            ...
            <input type="text" value={form.title} onChange={set("title")} placeholder="ex: DÃ©veloppeur Web Senior" className="flex-1 px-3 py-2.5 text-sm outline-none" maxLength={100} />
          </div>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">ExpÃ©rience</label>
          <select value={form.experienceLevel} onChange={set("experienceLevel")} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-teal-500 bg-white">
            <option value="no_experience">Sans expÃ©rience</option>
            <option value="entry">DÃ©butant (0-2 ans)</option>
            <option value="mid">IntermÃ©diaire (2-5 ans)</option>
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <label htmlFor="salary-neg" className="text-sm text-gray-600">Salaire nÃ©gociable</label>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="remote" className="text-sm text-gray-700">TÃ©lÃ©travail possible</label>
        </div>
      ...
      <div>
        ...
        <textarea value={form.description} onChange={set("description")} placeholder="DÃ©crivez le poste, les responsabilitÃ©s, l'environnement de travail..." rows={4} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-teal-500 resize-none" maxLength={2000} />
        <p className="text-xs text-gray-400 text-right mt-0.5">{form.description.length}/2000</p>
      </div>

      {/* Requirements */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Exigences & CompÃ©tences</label>
        <textarea value={form.requirements} onChange={set("requirements")} placeholder="DiplÃ´mes requis, compÃ©tences techniques, langues..." rows={3} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-teal-500 resize-none" maxLength={1000} />
      </div>
