...
const BambehLogo = ({ size = 40, className = '' }: { size?: number; className?: string }) => {
  ...
  if (imgError) return (
    <svg width={size} height={size} viewBox="0 0 40 40" className={className}>
      ...
      <text x="50%" y="54%" dominantBaseline="middle" textAnchor="middle" fill="white" fontSize="22" fontWeight="bold" fontFamily="system-ui">B</text>
    </svg>
  );
...
const mobileBottomNav = [
  ...
  { label: 'Notifs',    path: '/vendor/notifications',  icon: Bell },
...
const VendorLayout: React.FC<VendorLayoutProps> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (d) {
        ...
        const u = localStorage.getItem('bambe_current_user');
        if (u) {
          const p = JSON.parse(u);
          const n = p.displayName || p.username || 'Vendor';
          setVendorName(n);
          setVendorInitial(n.charAt(0).toUpperCase());
        }
      }
  ...
  const handleLogout = () => {
    [
      'Bambeh_vendor', 'Bambeh_current_vendor', 'Bambeh_vendor_redirect',
      'Bambeh_vendor_authenticated', 'Bambeh_vendor_auth_token',
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      window.location.href = '/vendor/listings?search=' + encodeURIComponent(searchQuery.trim());
      setSearchQuery('');
    }
  };
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-14">
              ...
              <div className="flex items-center gap-1.5">
                <Link to="/" className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                  <Home className="w-3.5 h-3.5" /><span>Marketplace</span>
                </Link>
                <Link to="/vendor/settings" className="p-2 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-all">
                  <Settings className="w-5 h-5" />
                </Link>
                <Link to="/vendor/notifications" className="p-2 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-all relative">
                    ...
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden" style={{ zIndex: 9999 }}>
                        ...
                        <div className="p-1.5">
                          ...
                          <Link to="/vendor/subscription" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg"><Crown className="w-4 h-4 text-amber-500" />Subscription</Link>
          ...
          <div className="lg:hidden bg-white border-t border-gray-100 shadow-xl max-h-[70vh] overflow-y-auto">
            <div className="px-4 py-3">
              <form onSubmit={handleSearch} className="mb-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => setSearchQuery(e.target.value)}
                    placeholder="Search..."
                    className="w-full pl-10 pr-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/40"
                  />
                </div>
              </form>
              <div className="space-y-1">
                {navTabs.map(tab => {
                  const Icon = tab.icon;
                  const a = isActivePath(tab.path);
                  return (
                    <Link
                      key={tab.path}
                      to={tab.path}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        a ? 'bg-teal-50 text-teal-700' : 'text-gray-700 hover:bg-gray-50'
                      }`}
                    >
                      <Icon className={`w-5 h-5 ${a ? 'text-teal-600' : 'text-gray-400'}`} />
                      <span>{tab.label}</span>
                    </Link>
                  );
                })}
              ...
              <div className="border-t border-gray-100 mt-3 pt-3 space-y-1">
                ...
                <Link to="/vendor/subscription" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-amber-50 to-orange-50 text-orange-700"><Crown className="w-5 h-5 text-amber-500" />Upgrade Plan</Link>
      ...
      <footer className="hidden md:block bg-white border-t border-gray-200 py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <BambehLogo size={32} />
              <span className="text-lg font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Bambeh Vendor</span>
            </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t-2 border-gray-100 shadow-2xl">
        <div className="flex items-center justify-around px-1 py-1.5">
          {mobileBottomNav.map(item => {
            ...
            const a = isActivePath(item.path);
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all min-w-0 ${
                  a ? 'text-teal-600' : 'text-gray-400 hover:text-teal-500'
                }`}
              >
                <Icon className={`w-5 h-5 ${a ? 'scale-110' : ''} transition-transform`} />
                <span className={`text-[10px] font-medium ${a ? 'text-teal-600' : 'text-gray-500'}`}>{item.label}</span>
              </Link>
            );
          })}
        </div>
