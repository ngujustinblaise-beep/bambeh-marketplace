...
const SERVICE_CATEGORIES = ["Informatique & Tech","Plomberie","?lectricit?","Peinture & D?coration","Jardinage","Nettoyage","Transport & D?m?nagement","Coiffure & Beaut?","Cours & Formation","Photographie","Design","Autres"] as const;
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div><label className="block text-sm font-medium text-gray-700 mb-1">Description <span className="text-red-500">*</span></label>
          <textarea value={form.description} onChange={e=>set("description",e.target.value)} rows={4} placeholder="D?crivez votre service en d?tail..." className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none resize-none" /></div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">Exp?rience</label>
          <input type="text" value={form.experience} onChange={e=>set("experience",e.target.value)} placeholder="Ex: 5 ans d'exp?rience en informatique" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none" /></div>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-sm text-gray-700">Disponible imm?diatement</span>
        </label>
