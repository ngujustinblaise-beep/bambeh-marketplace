...
const STATUS_CFG: Record<DisputeStatus, { label: string; color: string; icon: React.ElementType }> = {
  ...
  under_review:     { label: "En r�vision",          color: "text-yellow-600 bg-yellow-50 border-yellow-200", icon: Clock },
  resolved_buyer:   { label: "R�solu ? Acheteur",    color: "text-blue-600 bg-blue-50 border-blue-200",   icon: CheckCircle },
  resolved_seller:  { label: "R�solu ? Vendeur",     color: "text-teal-600 bg-teal-50 border-teal-200",   icon: CheckCircle },
  closed:           { label: "Ferm�",                color: "text-gray-500 bg-gray-100 border-gray-200",  icon: XCircle },
};
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-bold text-gray-900">R�solution des Litiges</h1>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center"><Scale className="w-8 h-8 text-gray-200 mx-auto mb-2" /><p className="text-sm text-gray-400">Aucun litige trouv�</p></div>
        ) : (
        ...
        <>
          ...
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl z-50 p-5 max-w-lg mx-auto space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-900">D�tail du litige</h3>
            ...
            {selected.status !== "closed" && selected.status !== "resolved_buyer" && selected.status !== "resolved_seller" && (
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">D�cision & Notes</label>
                  <textarea value={resolution} onChange={(e) => setResolution(e.target.value)} placeholder="Expliquez la d�cision prise..." rows={3} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-teal-500 resize-none" />
                </div>
