...
const STRINGS = {
  ...
  ar: {
    ...
    browseMore: 'ØªØµÙØ­ Ø§Ù„Ù…Ø²ÙŠØ¯ Ù…Ù† Ø§Ù„Ø¹Ù†Ø§ØµØ±',
    ...
    descLabel:  'Ø§Ù„ÙˆØµÙ *',
    minChars:   (n: number) => `${n} Ø­Ø±Ù Ø¹Ù„Ù‰ Ø§Ù„Ø£Ù‚Ù„`,
    descPlaceholder:'ØµÙ Ø¹Ù†ØµØ±Ùƒ â€” Ø§Ù„Ø­Ø§Ù„Ø©ØŒ Ø§Ù„Ø¹Ù…Ø±ØŒ Ø§Ù„Ù…Ù…ÙŠØ²Ø§ØªØŒ Ø§Ù„Ø¹ÙŠÙˆØ¨â€¦',
    chars:      (n: number) => `${n} Ø­Ø±Ù`,
    need:       (n: number) => ` â€” ÙŠØ­ØªØ§Ø¬ ${n} Ø£ÙƒØ«Ø±`,
    condition:  'Ø§Ù„Ø­Ø§Ù„Ø© *',
    estValue:   'Ø§Ù„Ù‚ÙŠÙ…Ø© Ø§Ù„ØªÙ‚Ø¯ÙŠØ±ÙŠØ© (ÙØ±Ù†Ùƒ)',
    estPlaceholder:'Ù…Ø«Ø§Ù„: 75000',
    howItWorks: 'ÙƒÙŠÙ ØªØ¹Ù…Ù„ Ø¹Ø±ÙˆØ¶ Ø§Ù„ØªØ¨Ø§Ø¯Ù„',
    step1:      'Ù‚Ø¯Ù‘Ù… Ø¹Ø±Ø¶Ùƒ Ù…Ø¹ ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ø¹Ù†ØµØ±',
    step2:      'ÙŠØ±Ø§Ø¬Ø¹ ØµØ§Ø­Ø¨ Ø§Ù„Ø¹Ù†ØµØ± Ø§Ù„Ø¹Ø±Ø¶ ÙˆÙŠÙ‚Ø¨Ù„Ù‡ Ø£Ùˆ ÙŠØ±ÙØ¶Ù‡',
    step3:      'Ø¹Ù†Ø¯ Ø§Ù„Ù‚Ø¨ÙˆÙ„ØŒ Ø£ØªÙ…Ù‘ Ø§Ù„ØµÙÙ‚Ø© Ø¹Ø¨Ø± Ø§Ù„Ù…Ø­Ø§Ø¯Ø«Ø©',
    step4:      'Ø§Ù„ØªÙ‚Ù ÙÙŠ Ù…ÙƒØ§Ù† Ø¹Ø§Ù… Ù„ØªØ¨Ø§Ø¯Ù„ Ø§Ù„Ø¹Ù†Ø§ØµØ±',
    cancel:     'Ø¥Ù„ØºØ§Ø¡',
    sendOffer:  'Ø¥Ø±Ø³Ø§Ù„ Ø§Ù„Ø¹Ø±Ø¶',
    sending:    'Ø¬Ø§Ø±Ù Ø§Ù„Ø¥Ø±Ø³Ø§Ù„â€¦',
    errOffering:'ÙŠØ±Ø¬Ù‰ Ø¥Ø¯Ø®Ø§Ù„ Ù…Ø§ ØªØ¹Ø±Ø¶Ù‡.',
    errDesc:    'ÙŠØ±Ø¬Ù‰ ÙˆØµÙ Ø¹Ù†ØµØ±Ùƒ.',
    errDescMin: 'ÙŠØ¬Ø¨ Ø£Ù† ÙŠØ­ØªÙˆÙŠ Ø§Ù„ÙˆØµÙ Ø¹Ù„Ù‰ 20 Ø­Ø±ÙÙ‹Ø§ Ø¹Ù„Ù‰ Ø§Ù„Ø£Ù‚Ù„.',
    errItem:    'Ø¹Ù†ØµØ± ØªØ¨Ø§Ø¯Ù„ ØºÙŠØ± ØµØ§Ù„Ø­.',
    errDuplicate:'Ù„Ù‚Ø¯ Ù‚Ø¯Ù‘Ù…Øª Ø¹Ø±Ø¶Ù‹Ø§ Ø¨Ø§Ù„ÙØ¹Ù„ Ø¹Ù„Ù‰ Ù‡Ø°Ø§ Ø§Ù„Ø¹Ù†ØµØ±.',
    errGeneric: 'ÙØ´Ù„ Ø§Ù„Ø¥Ø±Ø³Ø§Ù„. Ø­Ø§ÙˆÙ„ Ù…Ø¬Ø¯Ø¯Ù‹Ø§.',
  },
  pcm: {
