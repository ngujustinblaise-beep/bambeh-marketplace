 ...
 * FIXED: Reads from Supabase tontine_groups table with real group data.
 * Was a bare stub showing only the group ID.
 */
...
const DEMO_GROUP: Group = {
  id:'demo', name:'Demo Savings Group', description:'A sample tontine group. Create your own to get started.',
  adminId:'demo', contributionXaf:25000, frequency:'monthly', maxMembers:10,
  currentMembers:4, totalPoolXaf:100000, nextPayoutDate:'2026-06-15', status:'active',
};
...
export default function TontineDetail() {
  ...
  async function loadGroup(groupId: string) {
    ...
    try {
      ...
      if (dbErr || !data) { setError('Group not found.'); setLoading(false); return; }
      ...
      const { data: memberData } = await supabase
        ...
        .select('user_id, joined_at, payout_position, has_paid_current_round, profiles(full_name)')
        .eq('group_id', groupId)
  ...
  async function handleJoin() {
    ...
    if (!group || !isUUID(group.id)) return;
    setJoining(true);
    try {
      ...
      await supabase.from('tontine_groups').update({
  ...
  const fmt = (n: number) => `${n.toLocaleString()} XAF`;
  ...
  if (error || !group) return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="text-center">
        ...
        <button onClick={() => navigate('/tontine')} className="mt-4 text-purple-600 underline text-sm">
          Back to Tontine
        </button>
      </div>
    </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          {group.nextPayoutDate && (
            <div className="flex items-center gap-2 mt-3 text-xs text-teal-700 font-medium">
              ...
              Next payout: {new Date(group.nextPayoutDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
            </div>
          )}
      ...
      {!isMember && group.status === 'open' && group.currentMembers < group.maxMembers && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4">
          <button onClick={handleJoin} disabled={joining}
            className="w-full bg-purple-700 text-white py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-50">
            {joining
