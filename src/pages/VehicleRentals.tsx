 ...
 * FIXED: Reads from Supabase listings table (type='vehicle') instead of
 * localStorage + static SAMPLE_VEHICLES. Cross-device, real-time.
 */

import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  Search, MapPin, Gauge, Fuel, Plus, Car, Loader2, RefreshCw
} from 'lucide-react';
...
const SAMPLE_VEHICLES: Vehicle[] = [
  { id:'s1', title:'Toyota Camry 2020',         price:8500000,  location:'Yaoundé', category:'Sedan',  images:[], created_at: new Date().toISOString(), extra:{ fuel:'Petrol', transmission:'Automatic', mileage:'45,000 km' } },
  { id:'s2', title:'Honda Activa Motorcycle',    price:850000,   location:'Douala',  category:'Motorcycle', images:[], created_at: new Date().toISOString(), extra:{ fuel:'Petrol', transmission:'Manual', mileage:'12,000 km' } },
  { id:'s3', title:'Toyota Land Cruiser V8 2019',price:35000000, location:'Yaoundé', category:'SUV',    images:[], created_at: new Date().toISOString(), extra:{ fuel:'Diesel', transmission:'Automatic', mileage:'78,000 km' } },
  { id:'s4', title:'Nissan Pickup 4x4',          price:12000000, location:'Bamenda', category:'Pickup', images:[], created_at: new Date().toISOString(), extra:{ fuel:'Diesel', transmission:'Manual', mileage:'95,000 km' } },
];
...
export default function VehicleRentals() {
  ...
  async function fetchVehicles() {
    try {
      ...
      setVehicles(data && data.length > 0 ? data : SAMPLE_VEHICLES);
    } catch {
          ...
          <div className="space-y-4 pb-8">
            {filtered.map(v => (
              <div key={v.id}
                ...
                {/* Image */}
                <div className="h-44 bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center overflow-hidden">
                  {v.images?.[0]
