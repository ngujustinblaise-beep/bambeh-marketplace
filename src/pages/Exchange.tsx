 ...
 *
 * FIXED: Now reads from Supabase (visible to all devices/users)
 * instead of localStorage (was only visible on the posting device).
 * Real-time subscription so new posts appear instantly for everyone.
 */

import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeftRight, Plus, Package, Loader2, RefreshCw } from 'lucide-react';
...
export default function Exchange() {
  ...
  useEffect(() => {
    ...
    // ── Real-time: refresh when anyone posts a new item ──────────────────
    const channel = supabase
      .channel('exchange_items_changes')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'exchange_items' },
        () => { fetchItems(); }  // re-fetch when any row changes
      )
      .subscribe();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-teal-600 text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-1"
            >
        ...
        {!loading && !error && items.length === 0 && (
          <div className="text-center py-16 text-gray-500">
            ...
            <p className="font-medium mb-1">No exchange listings yet</p>
            <p className="text-sm text-gray-400 mb-4">Be the first to post an item for exchange!</p>
            <button
              onClick={() => navigate('/exchange/post')}
              className="bg-teal-600 text-white px-6 py-2 rounded-xl text-sm font-semibold"
            >
