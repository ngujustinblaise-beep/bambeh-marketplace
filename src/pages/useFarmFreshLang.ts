...
export const T = {
  ...
  joinGroup: {
    ...
    ar:       "انضم للشراء الجماعي — وفّر أكثر",
  ...
  loading: {
    ...
    ar:       "جارٍ تحميل المنتجات الطازجة…",
  ...
  photoSecure: { en:"📸 Photos are uploaded securely to Bambeh servers — not stored on your phone.", fr:"📸 Les photos sont téléchargées en toute sécurité sur les serveurs Bambeh — non stockées sur votre téléphone.", pidgin:"📸 Photos dey upload secure for Bambeh server — no dey save for your phone.", ar:"📸 يتم رفع الصور بأمان إلى خوادم Bambeh — ولا تُخزَّن على هاتفك.", fulfulde:"📸 Natal callinaa hukkaande e Bambeh server — alaa e telefon maa." },
  tapUpload:   { en:"Tap to upload photos of your produce", fr:"Appuyez pour télécharger des photos de votre produit", pidgin:"Tap here to upload photos of your thing", ar:"انقر لرفع صور منتجك", fulfulde:"Tap hanga callude natal ko'am maa" },
  ...
  charCount:   { en:(n:number)=>`${n} chars`, fr:(n:number)=>`${n} car.`, pidgin:(n:number)=>`${n} letters`, ar:(n:number)=>`${n} حرف`, fulfulde:(n:number)=>`${n} aranɗe` },
  ...
  confirmPay:    { en:(n:number)=>`Confirm & Pay ${n.toLocaleString("fr-CM")} XAF`, fr:(n:number)=>`Confirmer & Payer ${n.toLocaleString("fr-CM")} XAF`, pidgin:(n:number)=>`Confirm & Pay ${n.toLocaleString("fr-CM")} XAF`, ar:(n:number)=>`تأكيد ودفع ${n.toLocaleString("fr-CM")} XAF`, fulfulde:(n:number)=>`Hollu & Hol ${n.toLocaleString("fr-CM")} XAF` },
  ...
  enterPin:      { en:"A payment request was sent to your number. Enter your PIN to approve.", fr:"Une demande de paiement a été envoyée à votre numéro. Entrez votre PIN pour approuver.", pidgin:"Payment request don go your number. Put your PIN to approve.", ar:"تم إرسال طلب دفع إلى رقمك. أدخل رمز PIN للموافقة.", fulfulde:"Sariya ceede nawnaa e nomboro maa. Roŋku PIN maa hanga wallitde." },
  sendingRequest:{ en:"Sending payment request…", fr:"Envoi de la demande de paiement…", pidgin:"E dey send payment request…", ar:"جارٍ إرسال طلب الدفع…", fulfulde:"E nawna sariya ceede…" },
  ...
  waiting:          { en:(m:number,s:number)=>`Waiting… ${m}:${String(s).padStart(2,"0")}`, fr:(m:number,s:number)=>`Attente… ${m}:${String(s).padStart(2,"0")}`, pidgin:(m:number,s:number)=>`E dey wait… ${m}:${String(s).padStart(2,"0")}`, ar:(m:number,s:number)=>`جارٍ الانتظار… ${m}:${String(s).padStart(2,"0")}`, fulfulde:(m:number,s:number)=>`E ɗaɓɓi… ${m}:${String(s).padStart(2,"0")}` },
