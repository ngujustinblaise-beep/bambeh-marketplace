/**
 * src/utils/sendBookingMessage.ts â€” Bambeh Marketplace
 *
 * Shared helper called by all three booking modals:
 *   - BookVisitModal    (Rentals)   â†’ bookingType: 'visit'
 *   - BookServiceModal  (Services)  â†’ bookingType: 'service'
 *   - BookTestDrive     (Vehicles)  â†’ bookingType: 'test_ride'
 *
 * What it does:
 ...
 *   4. The reply input is hidden for that message â€” the conversation is one-way.
 *
 * The visitor never needs to see or dial the host's contact number â€”
...
const LABEL: Record<BookingType, string> = {
  visit:     'ðŸ“… Visit Request',
  test_ride: 'ðŸš— Test Ride Request',
  service:   'ðŸ”§ Service Booking Request',
};

/**
 * Sends a one-way in-app booking notification to the ad creator.
 * Safe to call even if the user is not logged in â€” returns silently.
 */
export async function sendBookingMessage(params: BookingMessageParams): Promise<void> {
  ...
  // Build the message body â€” the host sees everything they need
  const lines: string[] = [
    LABEL[bookingType],
    `Listing: ${adTitle}`,
    `Date: ${displayDate}`,
  ...
  try {
    ...
    // Insert the booking message â€” flagged so Chat.tsx renders it as a card
    ...
    // Log but don't crash the booking flow â€” the visit_request row was already saved
