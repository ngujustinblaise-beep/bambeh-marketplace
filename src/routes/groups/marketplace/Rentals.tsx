// BAMBEH_DEPLOY_TOKEN__RENTALS_FIX53_CLEAN
    ...
    try {
      ...
      if (rows.length > 0) {
        setProperties(rows.map((d) => ({
          ...
          isFurnished: !!(d.extra?.is_furnished ?? d.extra?.isFurnished ?? d.extra?.furnished ?? d.is_furnished),
          ...
          <div className="space-y-3">
            ...
            {filtered.map((p) => (
              <div
                ...
                className="bg-white rounded-2xl overflow-hidden shadow-sm border transition-all cursor-pointer hover:shadow-md active:scale-[0.99]"
              >
