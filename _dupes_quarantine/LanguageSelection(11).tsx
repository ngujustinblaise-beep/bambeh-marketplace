 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * FIX: persist the canonical "Bambeh_language" key that App.tsx's
 *      OnboardingFlowGuard and LanguageProvider both read, and navigate to the
 *      real "/terms-acceptance" route. Previously wrote "bambeh_user_lang"
 *      (which nothing reads) and navigated to "/terms" — causing the onboarding
 *      flow to bounce straight back to the language selector.
 */
...
// Canonical key — must match LANG_KEY in App.tsx and the guard's getItem.
...
export default function LanguageSelector() {
  ...
  const handleContinue = () => {
    // Persist the key the rest of the app actually reads.
    localStorage.setItem(STORAGE_KEY, selected);
    // Keep notifying live language consumers (useLang/useAppLang instances).
    window.dispatchEvent(
      new CustomEvent("bambeh:langchange", { detail: { language: selected } })
    );
    // Go to the real terms route (matches App.tsx <Route path="/terms-acceptance" />).
